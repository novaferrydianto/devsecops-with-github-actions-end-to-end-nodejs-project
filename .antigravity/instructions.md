...
- **CI/CD Pipeline:** GitHub Actions with SonarCloud (SAST), Snyk (SCA), Trivy (Image Scan + SBOM), OWASP ZAP (DAST), Gitleaks (Secret Scan), Cosign (Image & Release Signing), and Fuzz Testing (fast-check).
...
5. **Supply Chain:** Docker base images MUST be pinned to SHA256 digests. Never use floating tags (`:latest`) in production Dockerfiles. Require Cosign signing for all pushed images AND GitHub release assets.
